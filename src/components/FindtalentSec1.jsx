        ...
        <div className="form">
          <div className="for">
            <form onSubmit={handleSubmit}>
              <input type="text" name='name' placeholder='Your Full Name' className={`my-4 ps-2 ${error1}`} value={name} onChange={(e)=>{setName(e.target.value);handlecheck(e)}}/>
              <input  name="email" placeholder='Your Email' className={`mb-2 ps-2 ${error2}`} value={mail} onChange={(e)=>{setMail(e.target.value);handlecheck(e)}}/> <br />
              <div className="button"> <button className='btn btn-primary' type='submit'>{loading?<Loader/>:'Get Started'}</button> </div>  <br />
            </form>
          </div>
          <img src={dot} alt="" />
